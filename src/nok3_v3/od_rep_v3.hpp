...
#ifndef LBD_OD_REP_V3_HPP
#define LBD_OD_REP_V3_HPP
...
namespace sdsl {
    ...
    private:
        ...
        //t_ev od_tl;  //trip len
        //t_iv od_td;  //transfer description
        t_vv od_tld;   //transfer len and description
        size_type od_matrix_n;


        void copy(const od_rep_v3& o)
        {
    ...
    public:
        ...
        const t_vv &tld = od_tld;  //transfer len and description
        //const t_iv &td = od_td;  //transfer description
        ...
        //Copy constructor
        od_rep_v3(const od_rep_v3& o)
        {
            copy(o);
        }

        //Move constructor
        od_rep_v3(od_rep_v3&& o)
        {
            *this = std::move(o);
        }
        ...
        od_rep_v3(std::vector<std::vector<value_type>> &trips, size_type n){
            ...
            //t_iv od_tlq_(matrix_size - (n*n), 0);
            acum = 0;
            t_vv od_tld_;
            od_tld_.resize(od_pqq_.size());
            for(size_type o = 0; o < n; ++o) {
                for(size_type d = 0; d < n; ++d) {
                    acum += od_ways[{o,d}].size();
                    size_type m_id = ((n * o) + (d+1)) - 1;
                    ...
                    for(long t_id = od_ways[{o,d}].size() - 1, c = 1; t_id >= 0; --t_id, ++c) {
                        ...
                        od_tld_[pqq_id].resize(trip.size() - 2);
                        for(int idt = 1; idt < trip.size() - 1; ++idt) od_tld_[pqq_id][idt-1] = (trip[idt]);
                        //od_tlq_[pqq_id] = trip.size() - 2;    
                        /*std::cout << o+1 << ' ';
                        for(int jj = 0; jj < od_tld_[pqq_id].size(); ++jj) {
                            std::cout << od_tld_[pqq_id][jj] + 1 << ' ';
                        }
                        std::cout << d+1 << '\n';*/
                    }
                }
            ...
            // t_iv od_td_(od_tl_[od_tlq_.size()], 0);
            // for(size_type o = 0; o < n; ++o) {
                // for(size_type d = 0; d < n; ++d) {
                    // if(o == d) continue;
                    // 
...
// 
        ...
        t_vv get_trips(t_value o, t_value d) {
            ...
            int ts_id_s = od_matrix_rank0(bp_id_s);
            int ts_id_e = ts_id_s + (bp_id_e - bp_id_s);
            t_vv res(ts_id_e - ts_id_s);
            for(size_type id = ts_id_s, c = 0; id < ts_id_e; ++id, ++c) {
                int sz = od_tld[id].size();
                res[c].resize(sz+2);
                res[c][0] = o;
                for(value_type idd = 0; idd < sz; ++idd) {
                    res[c][idd+1] = od_tld[id][idd];
                }
                res[c][sz + 1] = d;
            }
            return res; 
        ...
        value_type get_people_quantity_linestop(size_type s) {
           ...
           // std::cout << "start: " << s << ' ' << od_matrix.size() - (od_pq.size() - 1) << ' '<<m_id_end <<  "\n";
            ...
            int size = od_tld.size();
            for(size_type ts_id = 0; ts_id < ts_id_s; ts_id++) {
                for(size_type td_id = 0; td_id < od_tld[ts_id].size(); ++td_id) {
                    if(od_tld[ts_id][td_id] == s) {
        ...
        void swap(od_rep_v3& o)
        {
            ...
            sdsl::util::swap_support(od_matrix_select, o.od_matrix_select,
                               &od_matrix, &(o.od_matrix));
            od_pq.swap(o.od_pq);
            od_tld.swap(o.od_tld);
            
            std::swap(od_matrix_n, o.od_matrix_n);
        }

        //Assignment Operator
        od_rep_v3& operator=(const od_rep_v3& o)
        {
            if (this != &o) {
                copy(o);
            }
        ...
        od_rep_v3& operator=(od_rep_v3&& o)
        {
            if (this != &o) {
                //Move elements
        ...
        size_type serialize(std::ostream& out, sdsl::structure_tree_node* v=nullptr, std::string name="")const
        {
            ...
            written_bytes += serialize_vector(od_tld, out, child, "tld");
