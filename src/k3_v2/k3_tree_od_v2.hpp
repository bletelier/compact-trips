...
#ifndef LBD_K3_TREE_OD_V2
#define LBD_K3_TREE_OD_V2
...
class k3_tree_od_v2: public k3_tree_base<>
{
    ...
    //******************* CONSTRUCTOR ***********************//
    //*******************************************************//
    k3_tree_od_v2() = default;

    k3_tree_od_v2(const k3_tree_od_v2& tr)
    {
        *this = tr;
    }

    k3_tree_od_v2(k3_tree_od_v2&& tr)
    {
        *this = std::move(tr);
    }

    k3_tree_od_v2(std::string filename, size_type size)
    {
...
//            std::cout << "Adding point " << pos_x << ", " << pos_y << ", " << pos_z << ")" << std::endl;
    ...
    k3_tree_od_v2(std::vector<std::vector<int>> &trips, size_type size) {
        ...
        int_vector<8> k_ts_(bv_size - (size*size), 0);	
	        ...
	        acum += od_ways[{x,y}].size();
                ...
                for(int j = 0; j < od_ways[{x,y}].size(); ++j) {
                    ...
                    for(int k = 1; k < od_ways[{x,y}][j].second.size() - 1; ++k) {
		                ...
		                transfer_map[{{o,d}, od_ways[{x,y}][j].second[k]}].push_back({k-1,tm_id + t2_add});
		            }
		            tm_id++;	
	            }
	        }
        ...
        int_vector<16> id(size_nv, 0);
        int_vector<16> k3_id(k_k3_l_rank(k_k3_l.size()), 0);
        int_vector<8> gr(size_nv, 0);
        ...
        k_id = int_vector<16>(id);
        k_nv_psum = enc_vector<>(qnv_psum);
        k_gr = int_vector<8>(gr);
        
        k_k3_id = int_vector<16>(k3_id);

        k_nv = t_bv(k_nv_);
    }
    
    
    k3_tree_od_v2(std::vector<point_type> &points, size_type size)
    {
        build(points, size);
    }
    ...
    k3_tree_od_v2& operator=(k3_tree_od_v2&& tr)
    {
        if (this != &tr) {
            k_t = std::move(tr.k_t);
            k_l = std::move(tr.k_l);
    ...
    //! Equal operator
    bool operator==(const k3_tree_od_v2& tr) const
    {
    ...
    unsigned int get_people_quantity_transfer(int s) {
        unsigned int res = 0;
        {
        ...
        }
        {
            ...
            for(int i = 0; i < points.size(); ++i) {
                ...
                int c = 0;
                int idd = *ids.begin();
                //std::cout << "ids: " << bp_id << ' ' << ts_id << '\n';
                for(int bpid = bp_id, tsid = ts_id; bpid < bp_id_e; ++bpid, ++tsid) {
                    //std::cout << idd << ' ' << res << ' ' << k_pq[ts_id] << ' ' << c << ' ' << ts_id << '\n';  
                    if(k_ts[tsid] > 1) {
                        if(c == idd) {
                            res += (k_pq[tsid+1] - k_pq[tsid]);
                            ids.erase(idd);
    ...
    bool exist_direct_trip(int o, int d) {
        if(o == d or o >= k_matrix_size or d >= k_matrix_size) return false;
        int m_id = ((k_matrix_size * o) + (d+1)) - 1;
        if(m_id <= 0) return false;
        unsigned int res = 0;
        int bp_id = k_bp_select(m_id) + 1;
        int ts_id = k_bp_rank(bp_id);
        if(k_ts[ts_id] == 0) return true;
        return false;
